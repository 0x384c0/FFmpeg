...
static struct Compressor *Compressor_new(unsigned int history)
{
...
static struct CompressorConfig *Compressor_getConfig(struct Compressor *obj)
{
        return &obj->prefs;
}

static void Compressor_delete(struct Compressor *obj)
{
	if (obj->peaks)
		free(obj->peaks);
	free(obj);
...
static void Compressor_Process_int16(struct Compressor *obj, int16_t *audio, unsigned int count){
