		...
		for(a=-radius; a<=radius; a++){
			...
			for(w=0;w<32;w++){
				for(q=0;q<32;q++){
					...
					if(thumb_prev[pos]!=thumb[q+w*32]){
						thumb_tmp[q+w*32]=255;
					}
				}
			}
			// filter this buffer
			for(w=0;w<32;w++){
				for(q=1;q<31;q++){
					pos=q+w*32;
					if(thumb_tmp[pos-1]==0 && thumb_tmp[pos+1]==0){
						thumb_tmp[pos]=0;
					}
				}
			}
			for(w=0;w<32;w++){
				for(q=1;q<31;q++){
					pos=q*32+w;
					if(thumb_tmp[pos-32]==0 && thumb_tmp[pos+32]==0){
						thumb_tmp[pos]=0;
					}
				}
			}
